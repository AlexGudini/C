...
*/
	...
	for (int i = 0; i < N; i++)
		for (int j = 0; j < neuronAmount; j++)
		{
			sumWeigth = sumWeigth + W[i][j]* W[i][j];
		}
	sumWeigth = sqrt(sumWeigth);

	for (int i = 0; i < N; i++)
		for (int j = 0; j < neuronAmount; j++)
		{
			W[i][j] = W[i][j] / sumWeigth;
		}
...
int main(int argc, char *argv[]) {
	...
	std::cout << std::endl << "input learing rate( 0 < rate <= 0.01): ";
	std::cin >> alpha_b;
	...
	//-----------------обучение на выборке из L образов----------------------------
	int iteration = 0;
	do {
		iteration++;
