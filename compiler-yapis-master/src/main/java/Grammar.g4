...
content : length | plusNames | print | functionCall | forCycle | elementDeclaration | listDeclaration | ifBlock | add | clear | remove | concate;
type : LIST | ELEMENT;
...
forCycle : FOR OPEN_BRACKET equalNames CLOSE_BRACKET block;




elementDeclaration : ELEMENT? NAME ASSIGNMENT (STRING | plusNames | NAME | get);
listDeclaration : (LIST NAME) | (LIST? NAME ASSIGNMENT NAME);
equalNames : NAME EQUAL NAME;
plusNames : NAME PLUS NAME;
length: LENGTH OPEN_BRACKET NAME CLOSE_BRACKET;
